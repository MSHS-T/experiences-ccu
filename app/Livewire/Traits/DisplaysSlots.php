...
trait DisplaysSlots
{
    ...
    protected function slotColor(Slot $slot): array
    {
        ...
        if (filled($slot->bookings)) {
            if ($slot->bookings->every(fn(Booking $booking) => $booking->confirmed)) {
                // Every booking confirmed : plateau color (100% opacity)
                return [
                    'backgroundColor' => $baseColor,
                    'borderColor'     => $baseColor
                ];
            } else {
                // At least one booking unconfirmed : striped plateau color (50% opacity) with 100% opacity
                return [
    ...
    {
        // No booking or all bookings confirmed : no prefix
        $prefix = (($slot->bookings->isEmpty() || $slot->bookings->every(fn(Booking $booking) => $booking->confirmed)) ? '' : '[?] ');
