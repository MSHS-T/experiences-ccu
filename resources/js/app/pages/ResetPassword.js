...
export default function ResetPassword(props) {
    ...
    const handleLogin = e => {
        ...
        recaptchaRef.current.executeAsync().then(token => {
            ...
            formData.append('token', queryStringParams.token);
            formData.append('email', queryStringParams.email);
            formData.append('password', data.password);
            formData.append('password_confirmation', data.password_confirmation);
            formData.append('g-recaptcha-response', token || '');

            // eslint-disable-next-line no-undef
            axios.post(Constants.API_URL + 'auth/resetpassword', formData)
                .then(() => {
                    setIsLoading(false);
                    setSuccess(true);
                })
                .catch(error => {
                    ...
                    case 422:
                        setError('Les mots de passe ne correspondent pas, ou ne respectent pas un minimum de 8 caractères. Veuillez vérifier votre saisie et réessayer.');
                        break;
                    case 400:
